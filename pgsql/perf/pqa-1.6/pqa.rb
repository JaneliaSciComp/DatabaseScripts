...
class GenericLogReader
  ...
  def parse
    ...
    File.foreach(@filename) {|text|
      begin 
        puts "Pre-Parse #{text}" if DEBUG
        line = p.parse(text)
        puts "Post-Parse #{line}" if DEBUG
        if line
          a.append(line)
        else
          # text.gsub!(/\n/, '\n').gsub!(/\t/, '\t')
...
class PostgreSQLParser
  ...
  QUERY_STARTER = Regexp.new("^(00000: query|00000: statement):[\s]*")
  ...
  def parse(text)
    ...
    if logdebug_match
      ...
      if status_match
       ...
       $stderr.puts "Unrecognized LOG or DEBUG line: #{text}" if DEBUG
      return nil
    end
...
class PostgresLogParser < PostgreSQLParser
  ...
  def parse(text)  
    connection_id = nil
    # $stderr.puts text
    text = STARTS_WITH_DATE.match(text) ? text.split(" ")[3..-1].join(" ").strip : text
    # $stderr.puts text
