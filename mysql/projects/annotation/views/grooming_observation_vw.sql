...
SELECT line.name, 
       ...
       MAX(IF(STRCMP(cv_term3.name,'interesting'),null,line_property.value)) AS 'interesting',
       MAX(IF(STRCMP(cv_term3.name,'expression_imaged'),null,line_property.value)) AS 'expression_imaged',
       MAX(IF(STRCMP(cv_term3.name,'experimental_imaged'),null,line_property.value)) AS 'experimental_imaged',
       MAX(IF(STRCMP(cv_term3.name,'evidence_imaged'),null,line_property.value)) AS 'evidence_imaged',
...
FROM line
JOIN line_property on (line.id = line_property.line_id)
JOIN cv_term cv_term3 on (line_property.type_id = cv_term3.id)
JOIN cv cv3 on (cv_term3.cv_id = cv3.id)
JOIN session on (line.id = session.line_id)
JOIN cv_term on (session.type_id = cv_term.id)
JOIN cv on (cv_term.cv_id = cv.id)
LEFT OUTER JOIN observation on (session.id = session_id)
LEFT OUTER JOIN cv_term cv_term2 on (observation.type_id = cv_term2.id)
WHERE cv.name = 'grooming'
  AND cv3.name in ('grooming','line')
GROUP BY line.name;
