...
DROP PROCEDURE IF EXISTS climbTree
//

CREATE DEFINER = annotationAdmin PROCEDURE climbTree
...
BEGIN
  ...
  -- show the result
  SELECT LPAD(name,length(name)+(select max(level) from tmp_adjacency_list) - level,'>') FROM tmp_adjacency_list WHERE connection = v_connection_id ORDER BY path desc;
