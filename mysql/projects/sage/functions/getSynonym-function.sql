...
DELIMITER //
CREATE DEFINER = sageAdmin PROCEDURE getSynonym
...
BEGIN
  ...
  SELECT cv_term.id FROM cv_term,cv WHERE cv.id = cv_term.cv_id AND cv_term.name ='has_synonym' AND cv.name = 'obo' INTO v_relationship_id;

  IF context is NULL THEN
    SELECT subject_id from cv_term_relationship,cv_term where subject_id = cv_term.id and cv_term.name = term and cv_term_relationship.type_id = v_relationship_id  LIMIT 1 INTO v_id;
    ...
    END IF;
  ELSE
    SELECT subject_id from cv_term_relationship,cv_term, cv where subject_id = cv_term.id and cv.id = cv_term.cv_id and cv_term.name = term and cv_term_relationship.type_id = v_relationship_id LIMIT 1 INTO v_id;
    IF v_id is NULL THEN
