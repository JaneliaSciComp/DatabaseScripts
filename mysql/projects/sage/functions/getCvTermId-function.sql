...
DELIMITER //
CREATE DEFINER = sageAdmin FUNCTION getCvTermId(context CHAR(255), term CHAR(255), definition CHAR(255))
RETURNS INT
DETERMINISTIC
BEGIN
  DECLARE v_cv_id int;
  ...
  IF v_cv_term_id is NULL THEN
    ...
    WHERE name = term collate latin1_general_cs 
      AND cv_id = object_id 
  ...
  IF v_cv_term_id is NULL THEN
    SELECT cv_term.id from cv_term, cv WHERE cv_term.name = 'has_synonym'  collate latin1_general_cs AND cv.name = 'obo' AND cv_term.cv_id = cv.id INTO v_relationship_id;
    select  id FROM cv_term WHERE name = term collate latin1_general_cs AND cv_id = v_cv_id AND is_current = 0 INTO v_cv_term_id;
...
GRANT EXECUTE ON FUNCTION sage.getCvTermId TO sageOlympiad@'localhost';
