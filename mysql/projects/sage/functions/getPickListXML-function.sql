DROP PROCEDURE IF EXISTS getPickListXML;
DELIMITER //
CREATE DEFINER = sageAdmin PROCEDURE getPickListXML
(OUT o_string text,column_name CHAR(255), table_name CHAR(255))
DETERMINISTIC
BEGIN

 DECLARE v_prefix text DEFAULT '<value><![CDATA[';
 DECLARE v_suffix text DEFAULT ']]></value>';
 ...
 OPEN values_cur;
   REPEAT
     FETCH values_cur INTO v_value;
       IF NOT done THEN
         SET v_string := CONCAT(v_string,v_prefix,v_value,v_suffix);
       END IF;
