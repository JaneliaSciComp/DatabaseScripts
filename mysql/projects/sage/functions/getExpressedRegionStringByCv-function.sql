DROP FUNCTION IF EXISTS getExpressedRegionStringByCv;
DELIMITER //
CREATE DEFINER = sageAdmin FUNCTION getExpressedRegionStringByCv
(p_line CHAR(255), p_cv CHAR(255))
RETURNS CHAR(255)
DETERMINISTIC
BEGIN
  ...
  DECLARE line_cur CURSOR FOR 
    ...
    WHERE session_vw.line = p_line collate latin1_general_cs
    ORDER BY term;
  ...
  OPEN line_cur;
    REPEAT
      ...
      IF NOT done THEN
        ...
        JOIN cv_term s on ( r.subject_id = s.id and s.name = v_name collate latin1_general_cs and s.is_current = 1)
        JOIN cv c on (s.cv_id = c.id and c.name = p_cv)
        JOIN cv_term o on ( r.object_id = o.id and o.is_current = 1)
