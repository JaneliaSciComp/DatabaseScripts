/* 
    ...
    mv:   olympiad_bubble_experiment_data_mv

    app:  SAGE REST API
    
    note: This script performs the following queries to flatten Bubble Assay data.
...
-- =================================================== 
DROP TABLE IF EXISTS tmp_olympiad_bubble_experiment_data_mv;
CREATE TABLE tmp_olympiad_bubble_experiment_data_mv
SELECT e.id AS experiment_id, cast(e.name as char(250)) AS experiment_name, 
       ...
       cast(protocol.value as char(50)) AS protocol, cast(led_protocol.value as char(50)) AS led_protocol, cast(screen_reason.value as char(50)) AS screen_reason, cast(screen_type.value as char(50)) AS screen_type, 
       cast(temperature.value as decimal(10,4)) AS temperature 
FROM experiment e LEFT JOIN experiment_property analysis_protocol     ON (analysis_protocol.experiment_id = e.id     AND analysis_protocol.type_id     = getCvTermId('fly_olympiad_fly_bubble', 'analysis_protocol', NULL))
                  LEFT JOIN experiment_property archived              ON (archived.experiment_id = e.id              AND archived.type_id              = getCvTermId('fly_olympiad_fly_bubble', 'archived', NULL))
                  ...
                  LEFT JOIN experiment_property data_capture_version  ON (data_capture_version.experiment_id = e.id  AND data_capture_version.type_id  = getCvTermId('fly_olympiad_fly_bubble', 'data_capture_version', NULL))
                  LEFT JOIN experiment_property experimenter          ON (experimenter.experiment_id = e.id          AND experimenter.type_id          = getCvTermId('fly_olympiad_fly_bubble', 'experimenter', NULL))
                  LEFT JOIN experiment_property exp_datetime          ON (exp_datetime.experiment_id = e.id          AND exp_datetime.type_id          = getCvTermId('fly_olympiad_fly_bubble', 'exp_datetime', NULL))
                  LEFT JOIN experiment_property file_system_path      ON (file_system_path.experiment_id =e.id       AND file_system_path.type_id      = getCvTermId('fly_olympiad_fly_bubble', 'file_system_path', NULL))
                  LEFT JOIN experiment_property flag_aborted          ON (flag_aborted.experiment_id = e.id          AND flag_aborted.type_id          = getCvTermId('fly_olympiad_fly_bubble', 'flag_aborted', NULL))
                  LEFT JOIN experiment_property flag_legacy           ON (flag_legacy.experiment_id = e.id           AND flag_legacy.type_id           = getCvTermId('fly_olympiad_fly_bubble', 'flag_legacy', NULL))
                  LEFT JOIN experiment_property flag_redo             ON (flag_redo.experiment_id = e.id             AND flag_redo.type_id             = getCvTermId('fly_olympiad_fly_bubble', 'flag_redo', NULL))
                  LEFT JOIN experiment_property flag_review           ON (flag_review.experiment_id = e.id           AND flag_review.type_id           = getCvTermId('fly_olympiad_fly_bubble', 'flag_review', NULL))
                  LEFT JOIN experiment_property humidity              ON (humidity.experiment_id = e.id              AND humidity.type_id              = getCvTermId('fly_olympiad_fly_bubble', 'humidity', NULL))
                  ...
                  LEFT JOIN experiment_property notes_behavioral      ON (notes_behavioral.experiment_id = e.id      AND notes_behavioral.type_id      = getCvTermId('fly_olympiad_fly_bubble', 'notes_behavioral', NULL))
                  ...
                  LEFT JOIN experiment_property notes_keyword         ON (notes_keyword.experiment_id = e.id         AND notes_keyword.type_id         = getCvTermId('fly_olympiad_fly_bubble', 'notes_keyword', NULL))
                  LEFT JOIN experiment_property notes_technical       ON (notes_technical.experiment_id = e.id       AND notes_technical.type_id       = getCvTermId('fly_olympiad_fly_bubble', 'notes_technical', NULL))
                  LEFT JOIN experiment_property protocol              ON (protocol.experiment_id = e.id              AND protocol.type_id              = getCvTermId('fly_olympiad_fly_bubble', 'protocol', NULL))
		  LEFT JOIN experiment_property led_protocol          ON (e.id = led_protocol.experiment_id          AND led_protocol.type_id          = getCvTermId('fly_olympiad_fly_bubble', 'led_protocol', NULL))
                  LEFT JOIN experiment_property screen_reason         ON (e.id = screen_reason.experiment_id         AND screen_reason.type_id         = getCvTermId('fly_olympiad_fly_bubble', 'screen_reason', NULL))
                  LEFT JOIN experiment_property screen_type           ON (e.id = screen_type.experiment_id           AND screen_type.type_id           = getCvTermId('fly_olympiad_fly_bubble', 'screen_type', NULL))
                  LEFT JOIN experiment_property temperature           ON (temperature.experiment_id = e.id           AND temperature.type_id           = getCvTermId('fly_olympiad_fly_bubble', 'temperature', NULL))
WHERE e.type_id = getCvTermId('fly_olympiad_fly_bubble', 'fly_bubble', NULL)
;
CREATE INDEX olympiad_bubble_edmv_experiment_id_ind ON tmp_olympiad_bubble_experiment_data_mv(experiment_id);


-- =================================================== 
-- create temp table containing session properties
-- =================================================== 
DROP TABLE IF EXISTS tmp_olympiad_bubble_session_meta_data_mv;
CREATE TABLE tmp_olympiad_bubble_session_meta_data_mv
SELECT e.id AS experiment_id, s.id AS session_id, cast(s.name as char(250)) AS session_name, s.line_id AS line_id, cast(l.name as char(50)) AS line_name, cast(lab_cv.name as char(50)) AS line_lab,
       cast(apparatus_id.value as char(255)) AS apparatus_id, cast(bubble.value as char(50)) AS bubble, cast(camera.value as char(50)) AS camera, 
       ...
       cast(rearing_protocol.value as char(50)) AS rearing_protocol, cast(rig.value as char(50)) AS rig, cast(robot_stock_copy.value as char(50)) AS robot_stock_copy,
...
FROM experiment e JOIN session s ON (s.experiment_id = e.id)
                  ...
                  LEFT JOIN session_property bubble                  ON (bubble.session_id = s.id                  AND bubble.type_id                  = getCvTermId('fly_olympiad_apparatus', 'bubble', NULL))
                  ...
                  LEFT JOIN session_property cross_barcode         ON (cross_barcode.session_id = s.id         AND cross_barcode.type_id         = getCvTermId('fly_olympiad_fly_bubble', 'cross_barcode', NULL))
                  ...
                  LEFT JOIN session_property flip_date             ON (flip_date.session_id = s.id             AND flip_date.type_id             = getCvTermId('fly_olympiad_fly_bubble', 'flip_date', NULL))
                  LEFT JOIN session_property flip_used             ON (flip_used.session_id = s.id             AND flip_used.type_id             = getCvTermId('fly_olympiad_fly_bubble', 'flip_used', NULL))
                  ...
                  LEFT JOIN session_property handler_cross         ON (handler_cross.session_id = s.id         AND handler_cross.type_id         = getCvTermId('fly_olympiad_fly_bubble', 'handler_cross', NULL))
                  ...
                  LEFT JOIN session_property robot_stock_copy      ON (robot_stock_copy.session_id = s.id      AND robot_stock_copy.type_id      = getCvTermId('fly_olympiad_fly_bubble', 'robot_stock_copy', NULL))
                  LEFT JOIN session_property room                  ON (room.session_id = s.id                  AND room.type_id                  = getCvTermId('fly_olympiad_apparatus', 'room', NULL))
                  LEFT JOIN session_property seconds_fliesloaded   ON (seconds_fliesloaded.session_id = s.id   AND seconds_fliesloaded.type_id   = getCvTermId('fly_olympiad_fly_bubble', 'seconds_fliesloaded', NULL))
                  LEFT JOIN session_property seconds_shiftflytemp  ON (seconds_shiftflytemp.session_id = s.id  AND seconds_shiftflytemp.type_id  = getCvTermId('fly_olympiad_fly_bubble', 'seconds_shiftflytemp', NULL))
...
WHERE e.type_id = getCvTermId('fly_olympiad_fly_bubble', 'fly_bubble', NULL)
;
CREATE INDEX olympiad_bubble_smd_mv_experiment_id_ind ON tmp_olympiad_bubble_session_meta_data_mv(experiment_id);

-- =================================================== 
-- create temp table combining experiment and session 
-- properties
-- =================================================== 
DROP TABLE IF EXISTS tmp_olympiad_bubble_data_mv;
CREATE TABLE tmp_olympiad_bubble_data_mv
SELECT e.experiment_id, e.experiment_name, s.line_id, s.line_lab, s.line_name,
       e.analysis_protocol, s.apparatus_id, e.archived, e.automated_pf, e.automated_pf_category, s.bubble, s.camera, 
       ...
       s.robot_stock_copy, s.room, e.led_protocol, e.screen_reason, e.screen_type, s.seconds_fliesloaded, 
       s.seconds_shiftflytemp, s.session_id, s.session_name, e.temperature, s.top_plate, 
       s.visual_surround, s.wish_list
FROM tmp_olympiad_bubble_experiment_data_mv  e
JOIN tmp_olympiad_bubble_session_meta_data_mv s ON (e.experiment_id = s.experiment_id);
CREATE INDEX olympiad_bubble_edmv_experiment_id_ind ON tmp_olympiad_bubble_data_mv(experiment_id);
CREATE INDEX olympiad_bubble_edmv_experiment_name_ind ON tmp_olympiad_bubble_data_mv(experiment_name);
CREATE INDEX olympiad_bubble_edmv_session_id_ind ON tmp_olympiad_bubble_data_mv(session_id);
CREATE INDEX olympiad_bubble_edmv_session_name_ind ON tmp_olympiad_bubble_data_mv(session_name);
CREATE INDEX olympiad_bubble_edmv_line_id_ind ON tmp_olympiad_bubble_data_mv(line_id);
CREATE INDEX olympiad_bubble_edmv_line_name_ind ON tmp_olympiad_bubble_data_mv(line_name);
CREATE INDEX olympiad_bubble_edmv_automated_pf_ind ON tmp_olympiad_bubble_data_mv(automated_pf);
CREATE INDEX olympiad_bubble_edmv_automated_pf_cat_ind ON tmp_olympiad_bubble_data_mv(automated_pf_category);
CREATE INDEX olympiad_bubble_edmv_exp_datetime_ind ON tmp_olympiad_bubble_data_mv(exp_datetime);
CREATE INDEX olympiad_bubble_edmv_flag_aborted_ind ON tmp_olympiad_bubble_data_mv(flag_aborted);
CREATE INDEX olympiad_bubble_edmv_manual_pf_ind ON tmp_olympiad_bubble_data_mv(manual_pf);
CREATE INDEX olympiad_bubble_edmv_led_protocol_ind ON tmp_olympiad_bubble_data_mv(led_protocol);
CREATE INDEX olympiad_bubble_edmv_screen_reason_ind ON tmp_olympiad_bubble_data_mv(screen_reason);
CREATE INDEX olympiad_bubble_edmv_screen_type_ind ON tmp_olympiad_bubble_data_mv(screen_type);

-- =================================================== 
-- create materialized view
-- =================================================== 
DROP TABLE IF EXISTS olympiad_bubble_experiment_data_mv;
RENAME TABLE tmp_olympiad_bubble_data_mv TO olympiad_bubble_experiment_data_mv;
DROP TABLE IF EXISTS tmp_olympiad_bubble_experiment_data_mv;
DROP TABLE IF EXISTS tmp_olympiad_bubble_session_meta_data_mv;
...
CREATE OR REPLACE VIEW olympiad_bubble_meta_data_vw AS 
SELECT * 
FROM olympiad_bubble_experiment_data_mv;

/* 
    olympiad_bubble_data_vw
...
CREATE OR REPLACE VIEW olympiad_bubble_data_vw AS
...
FROM olympiad_bubble_experiment_data_mv e
     JOIN score_array sa ON (sa.cv_id = getCvId('fly_olympiad_fly_bubble', NULL) AND sa.experiment_id = e.experiment_id)
     INNER JOIN cv_term ct ON (sa.type_id = ct.id)
WHERE ct.name NOT LIKE 'hist%';

/* 
    olympiad_bubble_data_vw2
...
CREATE OR REPLACE VIEW olympiad_bubble_data_vw2 AS
SELECT STRAIGHT_JOIN e.*,
...
CREATE OR REPLACE VIEW olympiad_bubble_score_data_vw AS
SELECT STRAIGHT_JOIN e.*,
       score_cv.name AS data_type,
       score.value AS data,
       1 AS data_rows, 
...
CREATE OR REPLACE VIEW olympiad_bubble_histogram_vw AS
SELECT STRAIGHT_JOIN e.*,
       getCvTermName(sa.type_id) AS data_type,
       uncompress(sa.value) AS data,
       sa.row_count AS data_rows, 
