...
#Database
my $SAGE = 'dbi:mysql:dbname=sage;host=db-dev';
my $dbh;
#Extras
my $is_current = 1;
my $data_type = "text";
...
my %sth = (
	SELECT => "SELECT id FROM cv WHERE name=?",
	SELECT_TERM => "SELECT name FROM cv_term WHERE name=? AND cv_id = ?",
	INSERT => "INSERT INTO cv_term (cv_id,is_current,data_type,name,display_name,definition) values (?,$is_current,'$data_type',?,?,?)"
);
...
while (defined($line = $stream->getline)) {
  chomp($line);
  my($cv,$term,$display,$definition) = split(/\t/, $line);
  if(exists $cv_hash{$cv}){
        $cv_id = $cv_hash{$cv};
  }
  else{
        $sth{SELECT}->execute($cv);
        $cv_id = $sth{SELECT}->fetchrow_array();
        if (defined ($cv_id)){
                $cv_hash{$cv} = $cv_id;
        }
        else{
                print $ehandle "ERROR: $cv is not a valid CV\n";
                next;
        }
  }
  $sth{SELECT_TERM}->execute($term,$cv_id);
  if ($sth{SELECT_TERM}->fetchrow_array()){
        print $ehandle "ERROR: $term already exists in the cv_term table for cv $cv\n";
        next;
  }
  $display ||= $term;
  $definition ||= $display;
  my($row_count) = $sth{INSERT}->execute($cv_id, $term, $display, $definition);
if(!$row_count){
        print $ehandle "ERROR: inserting $cv_id  $term  $display  $definition  into table.\n";
  }else{
 	$rows += 1; 
 }
 }

print $handle "$rows row(s) were inserted into the cv_term table.\n";
}
# ****************************************************************************
# * POD documentation                                                        *
# ****************************************************************************
